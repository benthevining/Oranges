...
if (("${CMAKE_SYSTEM_NAME}" MATCHES Linux)
    OR (NOT (PLAT_WIN OR PLAT_APPLE OR PLAT_ANDROID)))
...
if(UNIX OR PLAT_APPLE OR PLAT_LINUX)
    set (PLAT_UNIX 1 CACHE BOOL "True if the target plaform is Unix")
else()
    set (PLAT_UNIX 0 CACHE BOOL "True if the target plaform is Unix")
endif()
...
if (NOT ((DEFINED PLAT_ARM) OR (DEFINED CACHE{PLAT_ARM})
     OR (DEFINED PLAT_INTEL) OR (DEFINED CACHE{PLAT_INTEL})))
...
function(_oranges_plat_header_set_opts_for_language language)
    ...
    enable_language ("${ORANGES_ARG_LANGUAGE}")

    if (CMAKE_${language}_COMPILER_VERSION)
        set (PLAT_COMPILER_VERSION_${language} "${CMAKE_${language}_COMPILER_VERSION}" CACHE STRING "The compiler's full version string")

        string(REPLACE "." ";" version_list "${PLAT_COMPILER_VERSION}")
        ...
        if (version_major)
            set (PLAT_COMPILER_VERSION_MAJOR_${language} "${version_major}" CACHE STRING "Major component of the compiler's version")
        else()
            set (PLAT_COMPILER_VERSION_MAJOR_${language} 0 CACHE STRING "Major component of the compiler's version")
        endif()

        unset (version_major)

        if (version_minor)
            set (PLAT_COMPILER_VERSION_MINOR_${language} "${version_minor}" CACHE STRING "Minor component of the compiler's version")
        else()
            set (PLAT_COMPILER_VERSION_MINOR_${language} 0 CACHE STRING "Minor component of the compiler's version")
        endif()

        unset (version_minor)

        if (version_patch)
            set (PLAT_COMPILER_VERSION_PATCH_${language} "${version_minor}" CACHE STRING "Patch component of the compiler's version")
        else()
            set (PLAT_COMPILER_VERSION_PATCH_${language} 0 CACHE STRING "Patch component of the compiler's version")
        endif()

        unset (version_patch)
    else ()
        set (PLAT_COMPILER_VERSION_${language} Unknown CACHE STRING "The compiler's full version string")
        set (PLAT_COMPILER_VERSION_MAJOR_${language} 0 CACHE STRING "Major component of the compiler's version")
        set (PLAT_COMPILER_VERSION_MINOR_${language} 0 CACHE STRING "Minor component of the compiler's version")
        set (PLAT_COMPILER_VERSION_PATCH_${language} 0 CACHE STRING "Patch component of the compiler's version")
    endif ()
    ...
    if ("${compilerID}" MATCHES Clang)
        ...
        set (PLAT_COMPILER_TYPE_${language} "${compilerID}" CACHE STRING "${doc_plat_compiler_type}")
    else ()
    ...
    set_property (CACHE PLAT_COMPILER_TYPE_${language} PROPERTY STRINGS "Clang;AppleClang;GCC;MSVC;Intel;ARM;Cray;Unknown")
    ...
    if(NOT ((DEFINED PLAT_BIG_ENDIAN_${language}) OR (DEFINED CACHE{PLAT_BIG_ENDIAN_${language}}))
             AND ((DEFINED PLAT_LITTLE_ENDIAN_${language}) OR (DEFINED CACHE{PLAT_LITTLE_ENDIAN_${language}})))
        if(CMAKE_CROSSCOMPILING)
            message(WARNING "Cannot auto-detect endianness of current platform. Please fill the cache variables manually!")
            set (PLAT_BIG_ENDIAN_${language} 1 CACHE BOOL "${doc_plat_big_endian}")
            set (PLAT_LITTLE_ENDIAN_${language} 0 CACHE BOOL "${doc_plat_little_endian}")
        else()
            try_run (is_big_endian compile_result
                "${CMAKE_CURRENT_BINARY_DIR}/check_endian"
